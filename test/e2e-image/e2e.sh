...
if [ -z "${E2E_CHECK_LEAKS}" ]; then
  ginkgo_args+=("--skip=\[Admission\]|\[TopologyHints\]|\[Flag\]|\[Memory Leak\]")
else
  ginkgo_args+=("--skip=\[Admission\]|\[TopologyHints\]|\[Flag\]")
  # Drop serial tests, keeps Drone CI from hitting 1h timeout
fi
