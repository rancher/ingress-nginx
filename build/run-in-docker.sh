...
trap cleanup EXIT
E2E_RUNNER_VERSION=$(cat ./images/test-runner/TAG)
E2E_IMAGE=${E2E_IMAGE:-rancher/e2e-test-runner:$E2E_RUNNER_VERSION}
...
if [[ "$DOCKER_IN_DOCKER_ENABLED" == "true" ]]; then
  ...
  args="${PLATFORM_FLAG} ${PLATFORM} --user $(id -u):$(id -g) --tty --rm ${DOCKER_OPTS} -e DEBUG=${DEBUG} -e GOCACHE="/go/src/${PKG}/.cache" -e GOMODCACHE="/go/src/${PKG}/.modcache" -e DOCKER_IN_DOCKER_ENABLED="true" -v "${HOME}/.kube:${HOME}/.kube" -v "${KUBE_ROOT}:/go/src/${PKG}" -v "${KUBE_ROOT}/bin/${ARCH}:/go/bin/linux_${ARCH}" -v "${INGRESS_VOLUME}:/etc/ingress-controller/" -w "/go/src/${PKG}""
